...
try {
    ...
    if($App->_hiddenThirdpartyActive()){
      ...
      } else if($OrderInfos['thirdparty_internal_order'] == 'forex'){
          $CryptoApi = new CryptoApi($User, null, $App, 'FOREX');

          $OrderInfos['symbol_internal_order'] = ($OrderInfos['symbol_internal_order'] == 'USDT') ? 'USD' : $OrderInfos['symbol_internal_order'];
          $OrderInfos['to_internal_order'] = ($OrderInfos['to_internal_order'] == 'USDT') ? 'USD' : $OrderInfos['to_internal_order'];
      } else {
...
<header>
  <div>
    <div>
      ...
      <span><?php echo $OrderInfos['symbol_internal_order']; ?> / <?php echo $OrderInfos['to_internal_order']; ?></span>
...
<script>
  ...
  <?php
  if($OrderInfos['type_internal_order'] == "limit" && $OrderInfos['status_internal_order'] == 0):
  ?>
    <ul class="kr-orderinfoside-action">
      <li>
  ...
  <ul class="kr-orderinfoside-cinout">
    <?php if($OrderInfos['side_internal_order'] == "BUY"): ?>
      ...
      <li>
        <span>- <?php echo rtrim($App->_formatNumber($OrderInfos['amount_internal_order'], 8), "0"); ?></span>
        <label><?php echo $OrderInfos['to_internal_order']; ?></label>
      </li>
    <?php else: ?>
      <li>
        <span>+ <?php echo $App->_formatNumber($OrderInfos['usd_amount_internal_order'] - $OrderInfos['fees_internal_order'], 8); ?></span>
