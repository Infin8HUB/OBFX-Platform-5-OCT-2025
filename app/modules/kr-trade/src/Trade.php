...
class Trade extends MySQL {
  ...
  public function _getThirdPartyConfig(){
    return [
      ...
      'usstock' => [
          'key_usstock' => 'Usstock Key',
          'secret_usstock' => 'Usstock Secret',
          'sandbox' => null
        ],
      'forex' => [
          'key_forex' => 'Forex Key',
          'secret_forex' => 'Forex Secret',
          'sandbox' => null
        ],
    ];
  }
  ...
  public function _getThirdParty($params = null, $exchange = null){
    ...
    $this->ThirdParty = [
      ...
      'usstock' => new Usstock($this->_getUser(), $this->_getApp(), $params), 
      'forex' => new Forex($this->_getUser(), $this->_getApp(), $params),  
    ];
  ...
  public function _thirdparySymbolTrading($from, $to, $market = null){
    ...
    if(strtoupper($market) == "US_STOCK") $market = "usstock";
    if(strtoupper($market) == "FOREX") $market = "forex";
    ...
    if($this->_getApp()->_hiddenThirdpartyActive()){
      ...
      if(!is_null($search)){
        ...
        $req = parent::querySqlRequest("SELECT symbol_thirdparty_crypto, to_thirdparty_crypto, name_thirdparty_crypto, min_thirdparty_crypto,
                                        ...
                                        CONCAT(symbol_thirdparty_crypto, to_thirdparty_crypto) as pair_thirdparty_f FROM thirdparty_crypto_krypto 
                                        WHERE (".$selectedQuery.") 
                                        AND (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
                                        AND (to_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
        ...
        $req = parent::querySqlRequest("SELECT symbol_thirdparty_crypto, to_thirdparty_crypto, name_thirdparty_crypto, min_thirdparty_crypto,
                                        max_thirdparty_crypto, active_thirdparty_crypto, precision_amount_thirdparty_crypto,
                                        CONCAT(symbol_thirdparty_crypto, to_thirdparty_crypto) as pair_thirdparty_f 
                                        FROM thirdparty_crypto_krypto WHERE ".$selectedQuery." AND (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
                                        AND (to_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) ORDER BY FIELD(pair_thirdparty_f, ".$orderInfos.") DESC, FIELD(name_thirdparty_crypto, ".$selectedOrderExchange.") LIMIT ".$limit * 4, $selectedParams);
      }
      ...
      if(is_null($search)){
        $req = parent::querySqlRequest("SELECT * FROM thirdparty_crypto_krypto WHERE name_thirdparty_crypto=:name_thirdparty_crypto AND (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
                                        AND (to_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) LIMIT ".$limit,
                                      [
                                        'name_thirdparty_crypto' => $this->_getNameSelectedThirdPartyUser()
                                      ]);

        if(count($req) < $limit){
          $req = array_merge($req, parent::querySqlRequest("SELECT * FROM thirdparty_crypto_krypto WHERE (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
                                        AND (to_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1))  LIMIT ".($limit - count($req))));
        }
      } else {
        $infosSearch = explode('/', $search);
        if(count($infosSearch) == 2){
          $req = parent::querySqlRequest("SELECT * FROM thirdparty_crypto_krypto WHERE symbol_thirdparty_crypto=:symbol_thirdparty_crypto AND to_thirdparty_crypto=:to_thirdparty_crypto AND (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
                                        AND (to_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) LIMIT ".$limit,
                                        [
          ...
          $req = parent::querySqlRequest("SELECT * FROM thirdparty_crypto_krypto WHERE (name_thirdparty_crypto=:search_query_second OR symbol_thirdparty_crypto=:search_query OR to_thirdparty_crypto=:search_query) AND (symbol_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) 
                                        AND (to_thirdparty_crypto IN (SELECT symbol_coinlist FROM `coinlist_krypto` WHERE `status_coinslist` = 1)) LIMIT ".$limit, [
            'search_query' => $search,
        ...
        if(!array_key_exists($exchange.':'.$Market['symbol'], $marketListRes)){
          $r = parent::execSqlRequest("INSERT INTO thirdparty_crypto_krypto (symbol_thirdparty_crypto, to_thirdparty_crypto, name_thirdparty_crypto, min_thirdparty_crypto, max_thirdparty_crypto, precision_amount_thirdparty_crypto, active_thirdparty_crypto)
                                      ...
                                      [
                                        ...
                                        'min_thirdparty_crypto' => (!is_null($Market['info']['min_price']) ? $Market['info']['min_price'] : 0),
                                        'max_thirdparty_crypto' => (!is_null($Market['info']['max_price']) ? $Market['info']['max_price'] : 9999),
                                        'precision_amount_thirdparty_crypto' => (!is_null($Market['info']['decimal_places']) ? $Market['info']['decimal_places'] : 6),
  ...
  public function _checkLimitOrderNotCompleted() {
        ...
        foreach ($r as $key => $value) {
            ...
            if (!array_key_exists($value['id_user'], $UserCache))
                $UserCache[$value['id_user']] = new User($value['id_user']);
            $User = $UserCache[$value['id_user']];
            
            if($value['side_internal_order'] == 'SELL'){
                $OrderdValueWant = floatval($value['usd_amount_internal_order']) / floatval($value['amount_internal_order']);
            }
            ...
            // echo $value['side_internal_order'].' - OrderdValueWant:'.$OrderdValueWant . ' - Market Price:' . $Coin->_getPrice();
            
            if ($value['side_internal_order'] == 'SELL' && $OrderdValueWant <= $Coin->_getPrice() && $value['ordered_price_internal_order'] <= $Coin->_getPrice()) {
              // if ($value['side_internal_order'] == 'SELL' && $OrderdValueWant <= $Coin->_getPrice()) {
                $Ordered = true;
                var_dump('Upper : ' . $OrderdValueWant . ' - ' . $Coin->_getPrice());
            }
            // if ($value['side_internal_order'] == 'BUY' && $Coin->_getPrice() >= $OrderdValueWant && $value['ordered_price_internal_order'] >= $Coin->_getPrice()) {
              if ($value['side_internal_order'] == 'BUY' && $Coin->_getPrice() <= $OrderdValueWant) {
                $Ordered = true;
