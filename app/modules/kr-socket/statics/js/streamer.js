...
function parseMessage(data){
	...
	$('.c-marketprice-'+pair).attr('kr-market-multticker', currentPrice[pair]['PRICE']);
	$('.c-unitprice-'+pair).find("[kr-confirm-v='unit_price']").each(function() {
		$(this).attr("kr-confirm-v-up", currentPrice[pair]['PRICE']);
	});
	// console.log(pair+':'+currentPrice[pair]['PRICE']);
	...
	// console.log('CHANGE24HOUR', currentPrice[pair]['CHANGE24HOUR']);
	var chage24HourPrice = (currentPrice[pair]['PRICE'] - currentPrice[pair]['OPEN24HOUR']);
	if ($('#info-cur-'+pair).length) {
		$('#info-cur-'+pair).find('.kr-infoscurrencylf-price-cp').html((currentPrice[pair]['PRICE']).toFixed(2));
		if(chage24HourPrice >= 0){
			$('#info-cur-'+pair).find('.kr-infoscurrencylf-price-evolv').html((chage24HourPrice).toFixed(2) + '('+ currentPrice[pair]['CHANGE24HOURPCT'] +'%)');
			$('#info-cur-'+pair).find('.kr-infoscurrencylf-price-evolv').addClass('kr-hg-up');
		} else {
			$('#info-cur-'+pair).find('.kr-infoscurrencylf-price-evolv').html((chage24HourPrice).toFixed(2) + '('+ currentPrice[pair]['CHANGE24HOURPCT'] +'%)');
			$('#info-cur-'+pair).find('.kr-infoscurrencylf-price-evolv').addClass('kr-hg-down');
		}
	}
...
function addSubscribtion(symbol, currency, type = 5, market = "CCCAGG"){
    if(market == 'US_STOCK' || market == 'us_stock'){
        addStockSubscribtion(symbol);
    } else if(market == 'FOREX' || market == 'forex'){
...
function deleteSubscription(symbol, currency, type = 5, market = 'CCCAGG'){
    if(market == 'US_STOCK' || market == 'us_stock'){
        deleteStockSubscription(symbol);
    } else if(market == 'FOREX' || market == 'forex'){
        deleteForexSubscription(symbol);
    } else {
