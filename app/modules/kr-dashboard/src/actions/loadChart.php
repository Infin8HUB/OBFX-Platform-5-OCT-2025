...
try {
    ...
    $intervalTime = (isset($_GET['interval_time']) && $_GET['interval_time'] > 0) ? $_GET['interval_time'] : 1;
    ...
    if (empty($_GET['type']) || $_GET['type'] == "load") { // Load all graph
        ...
        if($intervalTime >= 1440){ // for 1 interval day //
            // Get list graph per days
            $CryptoGraphDays = new CryptoGraph($Coin->_getHistoDay(365, 1, false));
            $listCandles = $CryptoGraphDays->_getCandles();
            
        } else if($intervalTime >= 60){
            $interval = ($intervalTime / 60);
            $CryptoGraphHours = new CryptoGraph($Coin->_getHistoHour(720, $interval, false));
            $listCandles = $CryptoGraphHours->_getCandles();
        } else {
            // Init graph
            $interval = $intervalTime;
            $CryptoGraph = new CryptoGraph($Coin->_getHistoMin(1440, $interval));
            // List graph per hours
            $CryptoGraphHours = new CryptoGraph($Coin->_getHistoHour(720));
            $listCandles = array_merge($CryptoGraphHours->_getCandles(), $CryptoGraph->_getCandles());

            // Get list graph per days
            $CryptoGraphDays = new CryptoGraph($Coin->_getHistoDay(365));
            $listCandles = array_merge($CryptoGraphDays->_getCandles(), $listCandles);

            // $listCandles = $CryptoGraph->_compressCandle($listCandles, $intervalTime);
        }
        ...
        if($intervalTime >= 1440){ // for 1 interval day //
            // Get list graph per days
            $CryptoGraphDays = new CryptoGraph($Coin->_getHistoDay(365, 1, false));
            $listCandles = array_values(array_slice($CryptoGraphDays->_getCandles(), -1));
        } else if($intervalTime >= 60){ // for 1 interval day //
            $interval = ($intervalTime / 60);
            $CryptoGraphHours = new CryptoGraph($Coin->_getHistoHour(720, $interval, false));
            $listCandles = array_values(array_slice($CryptoGraphHours->_getCandles(), -1));
        } else {
            $interval = $intervalTime;
            $CryptoGraph = new CryptoGraph($Coin->_getHistoMin(100, $interval));
    ...
    $coinPrice = $Coin->_getCoinPrice($_GET['currency'] ,$Coin->_getSymbol());

    echo json_encode([
      'error' => 0,
      // 'candles' => $CryptoGraph->_compressCandle($listCandles, $intervalTime),
      'candles' => $listCandles,
