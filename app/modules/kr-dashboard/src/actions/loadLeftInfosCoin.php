    ...
    <header kr-leftinfoisp="<?php echo strtoupper($_REQUEST['market']).':'.$Coin->_getSymbol().$CryptoApi->_getCurrency(); ?>"
      ...
      <div class="kr-infoscurrencylf-header" style="<?php if($App->_getHideMarket()) echo 'min-height:13px;'; ?>">
        ...
        <?php
            ...
            ?>
      <div class="kr-infoscurrencylf-price left_info_<?php echo strtoupper($Coin->_getSymbol().$CryptoApi->_getCurrency()); ?>">
...
<?php
} else if($_REQUEST['market'] == 'FOREX'){
    $multiFullData = $Coin->_getForexAllMultiFullData();
    ?>
    <header kr-leftinfoisp="<?php echo strtoupper($_REQUEST['market']).':'.$Coin->_getSymbol().$CryptoApi->_getCurrency(); ?>"
        kr-leftinfois-makr="<?php echo strtoupper($_REQUEST['market']); ?>"
        kr-leftinfois-symbol="<?php echo strtoupper($Coin->_getSymbol()); ?>"
      ...
      <div class="kr-infoscurrencylf-header" style="<?php if($App->_getHideMarket()) echo 'min-height:13px;'; ?>">
        <h2><?= $Coin->_getSymbol().'/'.$CryptoApi->_getCurrency();?></h2>
        <?php if(!$App->_getHideMarket()): ?>
          <span class="market-span">CCCAGG</span>
        <?php endif; ?>
      </div>
        <?php
            $percentage = 0;
            $openDay = (isset($multiFullData['OPEN'])) ? $multiFullData['OPEN'] : '0.00';
            $currentForexPrice = (isset($multiFullData['CURRENT'])) ? $multiFullData['CURRENT'] : 0;
            $high24Hour = (isset($multiFullData['HIGH'])) ? $multiFullData['HIGH'] : 0;
            $low24Hour = (isset($multiFullData['LOW'])) ? $multiFullData['LOW'] : 0;
            $max = $high24Hour - $low24Hour;
            if($max == 0) { 
                ...
                $percentage = 100 - abs(((($currentForexPrice - $low24Hour) - $max) / $max) * 100);
            }
            
            $evolAmount = $currentForexPrice - $openDay;
            $evolPercentage = ($evolAmount * 100) / $currentForexPrice;
            $color = 'green';
            if($evolAmount < 0){
                $color = 'red';
            }
            ?>
      <div class="kr-infoscurrencylf-price left_info_<?php echo strtoupper($Coin->_getSymbol().$CryptoApi->_getCurrency()); ?>">
          <span class="kr-infoscurrencylf-price-cp_forex" style="font-size: 28px; font-weight: 700; line-height: 20px; margin-right: 10px; color: #c5cbce;"><?php echo (isset($multiFullData['CURRENT'])) ? $multiFullData['CURRENT'] : '0.00000' ?></span>
          <span class="kr-infoscurrencylf-price-evolv" style="color: <?= $color; ?>"><?= number_format($evolAmount, 2); ?> (<?= number_format($evolPercentage, 2) ?>%)</span>
      </div>
      <div class="kr-infoscurrencylf-range">
        <div class="kr-infoscurrencylf-range-bar">
            
            <div class="percentage_price_low_high_forex" style="width:<?= $percentage ?>%;">

          </div>
        </div>
        <div class="kr-infoscurrencylf-range-infos">
          <span class="kr-infoscurrencylf-range-infos-low_forex"><?php echo (isset($multiFullData['LOW'])) ? $multiFullData['LOW'] : '0.00' ?></span>
          <label><?php echo $Lang->tr('Day range'); ?></label>
          <span class="kr-infoscurrencylf-range-infos-high_forex"><?php echo (isset($multiFullData['HIGH'])) ? $multiFullData['HIGH'] : '0.00' ?></span>
        </div>
      </div>
      <ul>
        <!-- <?php
...
<?php    
    ...
    <header kr-leftinfoisp="<?php echo strtoupper($_REQUEST['market']).':'.$Coin->_getSymbol().$CryptoApi->_getCurrency(); ?>"
      ...
      <div class="kr-infoscurrencylf-price" id="info-cur-<?php echo $Coin->_getSymbol().$CryptoApi->_getCurrency(); ?>">
...
<?php
} else if(strtoupper($Coin->_getMarket()) == 'FOREX'){
  ?>
<script>
    $(document).ready(function(){
        $('header').attr('kr-leftinfoisp', '<?= $Coin->_getMarket() ?>'+':'+'<?= $_REQUEST['symbol'].$_REQUEST['currency'] ?>');
...
<script>
    $(document).ready(function(){
        $.ajax({url: "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=<?= $_REQUEST['symbol'] ?>&tsyms=<?= $_REQUEST['currency'] ?>", success: function(result){
