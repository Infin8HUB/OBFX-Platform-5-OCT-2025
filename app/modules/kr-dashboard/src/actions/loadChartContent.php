...
try {
  ...
  // echo "<pre>"; print_r($listThirdParty); exit;
...
<section class="kr-dash-pan-hedr">
  ...
  $selectedInterval = (isset($_GET['interval_time']) && $_GET['interval_time'] > 0) ? $_GET['interval_time'] : 1;
  ?>
  <div class="kr-dash-pan-hedr-ca">
    <div class="interval-indicator">
      <span><?php echo $Lang->tr($interval[$selectedInterval]); ?></span>
    </div>
    <ul class="kr-dash-pan-hedr-smli kr-dash-pan-interval">
      <?php
      foreach ($interval as $intervalKey => $intervalValue) {
        ?>
        <li interval_val="<?php echo $intervalKey; ?>"><span><?php echo $Lang->tr($intervalValue); ?></span></li>
        <?php
      }
      ?>
    </ul>
...
<div class="kr-dash-pan-action kr-dash-pan-action-limit" thirdparty="<?php echo $listThirdParty[0]->_getExchangeName(); ?>" container="<?php echo $container; ?>" currency="<?php echo $CryptoApi->_getCurrency(); ?>" symbol="<?php echo $Coin->_getSymbol(); ?>">
  ...
  <div class="kr-dash-pan-action-qtd c-marketprice-<?php echo $Coin->_getSymbol().$CryptoApi->_getCurrencySymbol(); ?>" kr-market-multticker="<?php echo $priceMarketUnit; ?>">
  ...
  <div class="kr-dash-pan-action-btn kr-dash-pan-action-btn-sell">
    ...
    <div class="kr-dash-pan-action-confirm">
      ...
      <ul>
        <li kr-order-lmi-h="true" class="c-unitprice-<?php echo $Coin->_getSymbol().$CryptoApi->_getCurrencySymbol(); ?>">
          <span><?php echo $Lang->tr('Unit price'); ?></span>
          <?php
          if(isset($_SESSION['is_manager_login']) && $_SESSION['is_manager_login'] == true){
              ?>
          <input type="number" id="" min="0" class="custom_price_unit" step="0" placeholder="0" name="custom_price_unit" value="0" style="display: none;">
          <span kr-confirm-v="unit_price" class="custom_unit_price_txt " kr-confirm-v-up="<?php echo $priceMarketUnit; ?>">
              <input type="number" id="" min="0" step="0" class="custom_unit_price_input" placeholder="0" name="" value="<?php echo $priceMarketUnit; ?>" style="width: 100px;border-radius: 2px;background: #353c4f;padding: 5px;color: #f4f6f9;border: none;outline: none;"> 
                  <?php echo $SymbolTrade; ?>
          </span>
          <span style="display: none;" kr-confirm-v="unit_price" class="custom_unit_price_span" kr-confirm-v-up="<?php echo $priceMarketUnit; ?>"><i><?php echo $App->_formatNumber($priceMarketUnit, ($priceMarketUnit > 10 ? 2 : 6)); ?></i> <?php echo $SymbolTrade; ?></span>
          <?php
          } else {
          ?>          
        ...
        <?php
        if((isset($_SESSION['is_manager_login']) && $_SESSION['is_manager_login'] == true) || (isset($_SESSION['maxleverage']) && $_SESSION['maxleverage'] > 1)){
//          if(isset($_SESSION['is_manager_login']) && $_SESSION['is_manager_login'] == true){
            $maxLeverage = ($_SESSION['is_manager_login'] == true) ? 100 : $_SESSION['maxleverage'];
              ?>  
        <li kr-order-lmi-s="true" class="margin_b">
          <span><?php echo $Lang->tr('Leverage'); ?></span>
          <div class="kr-margin-range-content" style="width: 60%;">
            <input type="hidden" class="<?php echo $randomClass; ?>_kr-margin-range-inp" name="margin_percentage" value="1">
            <div>
              <div class="kr-balance-range" kr-chosamount-precision="1">
                <input type="text" class="<?php echo $randomClass; ?>_kr-margin-chosamount" kr-chosamount-decimal="1" kr-chosamount-step="1" kr-chosamount-symbol="x" kr-chosamount-max="<?= $maxLeverage ?>" kr-chosamount-min="1" name="kr-credit-chosamount" value="1" />
              </div>
            </div>
          </div>
        </li>
  ...
  <div class="kr-dash-pan-action-limitprice kr-dash-pan-action-limitprice-selected">
    <div class="kr-dash-pan-action-limitprice-form" style="display: none;">
      <header>
        <span><?php echo $Lang->tr('Pending Order'); ?></span>
      </header>
      <div class="kr-dash-pan-action-limitprice-select">
        ...
        <div class="kr-dash-pan-action-limitprice-pm" kr-lm-container="<?php echo $container; ?>" kr-test="<?php echo $infosPairMarket['min_thirdparty_crypto'] ?>" kr-lm-min="<?php echo $infosPairMarket['min_thirdparty_crypto'] * $priceMarketUnit; ?>" kr-lm-step="1<?php //echo (float)$infosPairMarket['min_thirdparty_crypto'] * $priceMarketUnit; ?>">
          <div kr-lm="plus">+</div>
          <div kr-lm="minus">-</div>
        </div>
      </div>
    ...
    <div class="kr-dash-pan-action-limitprice-infos" style="cursor: pointer;" onclick="_showLimitOrder('<?php echo $container; ?>')">
...
<?php if($App->_tradingviewchartEnable() || ($App->_allowSwitchChart() && $User->_tradingviewChartLibraryUse())):
  ...
  ?>
<div class="tradingview-widget-container" style="height:100%;<?php if($availableTrading) echo 'margin-right:134px;'; ?>">
  ...
  tradingviewChart = new TradingView.widget(
      ...
      "interval": "1",
      "timezone": "Etc/UTC",
      "theme": "<?php echo ($User->_whiteMode() ? 'Light' : 'Dark'); ?>",
      "style": "1",
      "locale": "en",
      "toolbar_bg": "#fff",
      ...
      "backgroundColor": "<?php echo ($User->_whiteMode() ? '#fff' : '#353c4f'); ?>",
    }
  );
...
<script>
    ...
    $(document).ready(function(){   
        $(".<?php echo $randomClass; ?>_kr-margin-chosamount").ionRangeSlider({
          step: $('.<?php echo $randomClass; ?>_kr-margin-chosamount').attr('kr-chosamount-step'),
          grid: true,
          min:$(".<?php echo $randomClass; ?>_kr-margin-chosamount").attr('kr-chosamount-min'),
          max:$(".<?php echo $randomClass; ?>_kr-margin-chosamount").attr('kr-chosamount-max'),
          postfix: ' ' + $('.<?php echo $randomClass; ?>_kr-margin-chosamount').attr('kr-chosamount-symbol'),
          grid_num:1,
          onChange: function (data) {
            $('.<?php echo $randomClass; ?>_kr-margin-range-inp').val(data.from);
          }
        }); 
    });
