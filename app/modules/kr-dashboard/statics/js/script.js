...
function initDashboard(onlyGraphProperty = false){
  ...
  $('.kr-dash-pan-cry').each(function(){
    ...
    if(symbol != "not_init"){
      ...
      var topItem = $('.kr-top-graphlist-item[container="' + idPan + '"]').attr('topitem');
      var intervalVal = localStorage.getItem('interval-'+idPan+'-'+topItem);
      if(intervalVal == null){
        intervalVal = 1;
      }
      console.log('intervalVal-'+topItem, intervalVal);
      rangeGraph = intervalVal * 30;
      
      // Init graph
      loadChart(symbol, function(){
        loadChartData(symbol, idPan);
      }, idPan, currency, market, intervalVal, rangeGraph);
    }
  ...
  subscribeStreamerCallback(function(dataMessage){
    ...
    $('.kr-top-graphlist-item[symbol="' + dataMessage.FROMSYMBOL + '"][currency="' + dataMessage.TOSYMBOL + '"]').find('[kr-data="CHANGE24HOURPCT"]').html(dataMessage.CHANGE24HOURPCT + "%");
    $('.kr-top-graphlist-item[symbol="' + dataMessage.FROMSYMBOL + '"][currency="' + dataMessage.TOSYMBOL + '"]').attr('kr-val-graph', dataMessage.PRICE);
    $('.kr-top-graphlist-item[symbol="' + dataMessage.FROMSYMBOL + '"][currency="' + dataMessage.TOSYMBOL + '"]').find('[kr-data="CHANGE24HOURPCT"]').removeClass('kr-top-graphlist-item-evl-up').removeClass('kr-top-graphlist-item-evl-down')
    if(parseFloat(dataMessage.CHANGE24HOURPCT) < 0){
      $('.kr-top-graphlist-item[symbol="' + dataMessage.FROMSYMBOL + '"][currency="' + dataMessage.TOSYMBOL + '"]').find('[kr-data="CHANGE24HOURPCT"]').addClass('kr-top-graphlist-item-evl-down');
    } else if(parseFloat(dataMessage.CHANGE24HOURPCT) > 0){
      $('.kr-top-graphlist-item[symbol="' + dataMessage.FROMSYMBOL + '"][currency="' + dataMessage.TOSYMBOL + '"]').find('[kr-data="CHANGE24HOURPCT"]').addClass('kr-top-graphlist-item-evl-up');
    }
  });
...
function reloadTopListGraph(){
  ...
  $('.kr-top-graphlist-item').each(function(){
    ...
    $(this).off('click').click(function(){
      ...
      } else if($('[kr-module="stock"][kr-view="dashboard"].kr-leftnav-select').length > 0){
        if(!$('[kr-view="dashboard"]').hasClass('kr-leftnav-select')){
          changeView('stock', 'dashboard', {}, changeTopItemAction($(this)));
        } else {
          changeTopItemAction($(this));
        }
      } else if($('[kr-module="forex"][kr-view="dashboard"].kr-leftnav-select').length > 0){
        if(!$('[kr-view="dashboard"]').hasClass('kr-leftnav-select')){
          changeView('forex', 'dashboard', {}, changeTopItemAction($(this)));
        } else {
          changeTopItemAction($(this));
        }
      } else {
