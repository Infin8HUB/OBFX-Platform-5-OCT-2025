...
function initTradingChartControllers(){
  ...
  $(document).mouseup(function(e)
  {
    ...
    if (!container.is(e.target) && container.has(e.target).length === 0) {
      $('.kr-dash-pan-action-limitprice-form').hide();
      if($('.kr-dash-pan-action-limitprice-infos').find('span').text() == ''){
          $('.kr-dash-pan-action-limitprice-infos').find('span').html('Pending Order');
      }
    }
...
function _showLimitOrder(container){
  ...
  if($('.kr-dash-pan-action[container="' + container + '"]').find('.kr-dash-pan-action-limitprice-form').find('.kr-limitprice-buy').val() != ''){
    let currentAmount = parseFloat($('.kr-dash-pan-action[container="' + container + '"]').find('.kr-dash-pan-action-limitprice-form').find('.kr-limitprice-buy').val());
    currentAmount = parseFloat(currentAmount).toFixed(parseInt(4));
    $('.kr-dash-pan-action[container="' + container + '"]').find('.kr-dash-pan-action-limitprice-infos').find('span').html(currentAmount);
  }
}

function _hideLimitOrder(container){
  $('.kr-dash-pan-action[container="' + container + '"]').find('.kr-dash-pan-action-limitprice-form').hide();
  $('.kr-dash-pan-action[container="' + container + '"]').find('.kr-dash-pan-action-limitprice-infos').find('span').html('Pending Order');
}
...
//    confirmationBox.find('.custom_unit_price_span').show();
//    confirmationBox.find('.custom_unit_price_txt').hide();
    if(confirmationBox.find(".custom_unit_price_txt").length){
        confirmationBox.find('.custom_unit_price_input').val(confirmationBox.find('.custom_unit_price_span').attr('kr-confirm-v-up'));
        confirmationBox.find('.custom_price_unit').val('0');
    }
  } else {
      $('.margin_b').show();
    ...
    if(confirmationBox.find(".custom_unit_price_txt").length){
//        confirmationBox.find('.custom_unit_price_span').hide();
//        confirmationBox.find('.custom_unit_price_txt').show();
        confirmationBox.find('.custom_unit_price_input').val(confirmationBox.find('.custom_unit_price_span').attr('kr-confirm-v-up'));
        confirmationBox.find('.custom_price_unit').val('0');
    }
  }
  ...
  confirmationBox.find('.custom_unit_price_input').on('change', function(){
      confirmationBox.find('.custom_price_unit').val($(this).val());
  })
}
...
function updatePriceAmountTrading(){
  $('.kr-dash-pan-action').each(function(){
    ...
    $(this).find('[kr-confirm-v="unit_price"]').find('i').html(KRformatNumber(priceValue, 6));
...
function placerOrder(container, side, market){
    ...
    // }
    //
    let customUnitPrice = tradingActionContainer.find('input[name="custom_price_unit"]').val();
    let marginPercentage = tradingActionContainer.find('input[name="margin_percentage"]').val();
