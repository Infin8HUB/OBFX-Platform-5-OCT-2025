...
function applyOrderGraphList(orders, native = false){
  ...
  $.each(orders, function(k, v){
    ...
    // changes on 31-10-2019 //
    var boughtMarginHtml = '';
    if(v.side == "BUY" && v.is_sold == 1 && v.margin > 1){
        var boughtMarginHtml = '<span style="font-size: 12px;color: green;text-transform: lowercase;"><br>(bought with x'+v.margin+')</span>';
    }
    
    var soldMarginHtml = '';
    if(v.side == "SELL" && v.margin > 1 && v.margin > 1){
        var soldMarginHtml = '<span style="font-size: 12px;color: red;text-transform: lowercase;"><br>(soldout with x'+v.margin+')</span>';
    }
    
    marginHtml = '';
    if(v.margin_val > 1){
        var marginHtml = '<span class="sell_margin_div_'+v.id+'" style="font-size: 12px;color: red;text-transform: lowercase;"><br>(sell with x'+v.margin_val+')</span>';
    }
    var dotHtml = dotHtml = "<span class='dot_label_"+v.id+"' style='width: 10px;height: 10px;background-color: transparent;margin-right: 5px;border: 1px solid transparent;border-radius: 100%;display: inline-block;margin-left: 4px;'>&nbsp;</span>";
    if(v.is_show == 0 && v.manager_login){
        dotHtml = "<span class='dot_label_"+v.id+"' style='width: 10px;height: 10px;background-color: red;margin-right: 5px;border: 1px solid red;border-radius: 100%;display: inline-block;margin-left: 4px;'>&nbsp;</span>";
    }
    
    var liHtml = "<li style='height:auto;' kr-orderlist-i='" + v.id + "' kr-orderlist-pair='" + v.symbol + v.currency + "'> "+dotHtml+" <div>" + (orderDate.getDate() < 10 ? "0" + orderDate.getDate() : orderDate.getDate()) + "/" + ((orderDate.getMonth() + 1) < 10 ? "0" + (orderDate.getMonth() + 1) : (orderDate.getMonth() + 1) ) +
                        ...
                        "</div> <div>" + v.symbol + "/" + v.currency + "</div>" + " <div class='" + (v.side == "BUY" ? "kr-dash-orderlistpassed-lst-cgreen" : "kr-dash-orderlistpassed-lst-cred") + "'>" + v.side + marginHtml + boughtMarginHtml + soldMarginHtml + "</div>" +
                        "<div>" + minusQtd + "</div> <div>" + winQtd + "</div> <div> "+v.details+" </div> <div>" + v.evolv + "</div>" ;
    
//    var liHtml = "<li kr-orderlist-i='" + v.id + "' kr-orderlist-pair='" + v.symbol + v.currency + "'> "+dotHtml+" <div>" + (orderDate.getDate() < 10 ? "0" + orderDate.getDate() : orderDate.getDate()) + "/" + ((orderDate.getMonth() + 1) < 10 ? "0" + (orderDate.getMonth() + 1) : (orderDate.getMonth() + 1) ) +
//                        "/" + orderDate.getFullYear() +
//                        " " + (orderDate.getHours() < 10 ? "0" + orderDate.getHours() : orderDate.getHours()) + ":" + (orderDate.getMinutes() < 10 ? "0" + orderDate.getMinutes() : orderDate.getMinutes()) + ":" + (orderDate.getSeconds() < 10 ? "0" + orderDate.getSeconds() : orderDate.getSeconds()) +
//                        "</div> <div>" + v.symbol + "/" + v.currency + "</div>" +
//                        "<div>" + v.type.toUpperCase() +"</div>" +
//                        "" + (!native ? "<div>" + v.exchange + "</div>" : "") + " <div class='" + (v.side == "BUY" ? "kr-dash-orderlistpassed-lst-cgreen" : "kr-dash-orderlistpassed-lst-cred") + "'>" + v.side + marginHtml + boughtMarginHtml + soldMarginHtml + "</div>" +
//                        "<div>" + minusQtd + "</div> <div>" + winQtd + "</div> <div>" + KRformatNumber(v.fees, 8) + " " + winSymbol + "</div> <div>" + (KRformatNumber((v.side == "BUY" ? v.amount : v.usd_amount) - v.fees, 8)) + " " + winSymbol + "</div> <div>" + v.evolv + "</div>" ;
    if(v.manager_login){
        var btnName = (v.is_show == 1) ? "Hide" : "Show";
        liHtml += "<div><input type='button' id='hide_show_btn_"+v.id+"' onclick='_hideShowOrder(\"" + v.id + "\");return false;' class='btn btn-exsmall btn-grey hide_show_btn_"+v.id+"' value='"+btnName+"'/></div>";
    }
    liHtml += "</li>";
    
    let orderObject = $(liHtml);
