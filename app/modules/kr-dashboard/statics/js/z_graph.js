...
function changeGraph(dataCoin, currency, container, topitem, replace = false, market="CCCAGG"){
  ...
  var intervalVal = localStorage.getItem('interval-'+container+'-'+topItem);
  if(intervalVal == null){
    intervalVal = 1;
  }
  ...
  loadChart(dataCoin.symbol, null, container, currency, market, intervalVal, rangeGraph);
  if(replace){      
 ...
 * @param  {String}   intervalTime Interval time (in minitue)
 */
function loadChart(symbol, callback, container, currency, market = "CCCAGG", intervalTime = 1, rangeGraph = 30){
  ...
  // Load chart content
  $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChartContent.php', {coin:symbol, container:container, currency:currency, market:market, interval_time:intervalTime}).done(function(data){
    ...
    try { // Try to parse data in JSON -> true = error
      ...
      $.when($('#' + container).html(data)).then(function(){ // Add graph in container & load data
        ...
        loadChartData(symbol, $('#' + container).attr('currency'), $('#' + container).attr('market'), function(data){
          ...
          syncChart(container, intervalTime);
          ...
          initRangePan(container, rangeGraph);
 ...
 */
function loadChartData(symbol, currency, market, fnc = null, container, intervalTime = 1){
  // Get chart data
  $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChart.php', {coin:symbol, currency:currency, market:market, type:'load', interval_time:intervalTime}).done(function(data){
...
function syncChart(container, intervalTime = 1){
  // console.log('intervalTime:', intervalTime);
  intervalContainer[container] = intervalTime;
  if($('#' + container).length == 0) return false;
  let symbol = $('#' + container).attr('symbol');
  // console.log('intervalContainer[container]:', intervalContainer[container]);
  ...
  // Get graph data
  $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChart.php', {coin:symbol, type:'update', market:$('#' + container).attr('market'), currency:$('#' + container).attr('currency'), interval_time:intervalTime}).done(function(data){
    ...
    syncContainer[container] = setTimeout(function(){
      if($('#' + container).attr('symbol') != "not_init"){
        syncChart(container, intervalContainer[container]);
      }
    }, 3000);
  }).fail(function(){
