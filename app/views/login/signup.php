...
<?php if ($App->_visitorAllowedRegister()): ?>
<section class="kr-login-act" act="<?php echo APP_URL; ?>/app/modules/kr-user/src/actions/signup.php">
  <section class="kr-login-field">
    ...
    <div class="kr-user-f-l">
      <div style="width: 18%">
        <select name="kr_usr_country" id="countries" style="width: 100%">
          <option value="US" data-callingCode="1">United States</option>
          <option value="GB" data-callingCode="44">United Kingdom</option>
          <option value="AE" data-callingCode="971">United Arab Emirates</option>
          <option value="CY" data-callingCode="357">Cyprus</option>
          <option value="TR" data-callingCode="90">Turkey</option>
          <option value="IN" data-callingCode="91">India</option>
        </select>
    ...
    <footer>
      ...
      <?php if ($App->_captchaSignup()): ?>
      <button class="btn-shadow" data-sitekey="" data-size="" data-callback="kryptoSignup">
        <?php echo strtoupper($Lang->tr("Let's go !")); ?>
      </button>
      <?php else: ?>
...
<section class="kr-login-act-msg">
  ...
  <ul>
    ...
    <?php endif; ?>
    <?php if(strlen($App->_getSupportPhone()) > 1): ?>
...
$(document).ready(function() {
  $('#countries').on('change', function () {
    var callingCode = $(this).find(':selected').data('callingcode');
    $('#lcp-ccode').val(callingCode);
    $('#calling_code').val($(this).val());
  }).change();
});
</script>
