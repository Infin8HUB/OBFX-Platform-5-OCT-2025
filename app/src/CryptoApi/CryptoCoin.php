...
class CryptoCoin extends MySQL {
  ...
  private $StockMultifull = null;
  private $ForexMultifull = null;

  private $Market = null;

  private $ForexApi = null;
  ...
  public function _loadData(){
    if($this->_getMarket() == 'US_STOCK'){
      // echo  $this->_getSymbol().$this->ForexApi->_getCurrency();
        $valCoin = parent::querySqlRequest("SELECT * FROM stocklist_krypto WHERE symbol=:symbol AND status=:status", ['symbol' => $this->_getSymbol(), 'status' => 1]);
        if(count($valCoin) == 0) throw new Exception("Error : Unable to load stock (".$this->_getSymbol().")", 1);
        $valCoin = $valCoin[0];
        $this->CryptoCoinData = [
            'Id' => $valCoin['id'],
            'Url' => $valCoin['displaySymbol'],
            'Name' => $valCoin['symbol'],
            'CoinName' => $valCoin['symbol'],
            'FullName' => $valCoin['description'],
            'Algorithm' => '-',
            'ProofType' => '-',
            'SortOrder' => $valCoin['id'],
            'Symbol' => $valCoin['symbol'],
            'Status' => 1,
            'Source' => 'US_STOCK',
            'figi' => $valCoin['figi'],
            'mic' => $valCoin['mic'],
    ...
    } else if(strtoupper($this->_getMarket()) == 'FOREX'){
        $currency = ($this->ForexApi->_getCurrency() == 'USDT') ? 'USD' : $this->ForexApi->_getCurrency();
        // echo "SELECT * FROM forexlist_krypto WHERE symbol=".$this->_getSymbol()." AND currency =".$currency." AND status = 1"; 
        $valCoin = parent::querySqlRequest("SELECT * FROM forexlist_krypto WHERE symbol=:symbol AND status=:status", ['symbol' => $this->_getSymbol(), 'status' => 1]);
        // echo "<pre>"; print_r($valCoin); exit;

        if(count($valCoin) == 0){
          // echo "SELECT * FROM forexlist_krypto WHERE currency='".$this->_getSymbol()."' AND symbol='".$currency."' AND status=1";
          $valCoin = parent::querySqlRequest("SELECT * FROM forexlist_krypto WHERE currency=:currency AND symbol=:symbol AND status=:status", ['currency' => $this->_getSymbol(), 'symbol' => $currency, 'status' => 1]);
        }

        if(count($valCoin) == 0) throw new Exception("Error : Unable to load forex (".$this->_getSymbol().")", 1);
        $valCoin = $valCoin[0];
        $this->CryptoCoinData = [
            'Id' => $valCoin['id'],
            'Url' => $valCoin['displaySymbol'],
        ...
        // echo "<pre>"; print_r($this->CryptoCoinData);
    } else {
   ...
   */
  private function _getHistoCoin($type, $limit, $interval = 1, $is_slice = true){
    ...
    $getHistoCache = parent::querySqlRequest("SELECT * FROM histo_krypto WHERE coin_histo=:coin_histo AND currency_histo=:currency_histo AND type_histo=:type_histo
                                              AND last_update_histo=:last_update_histo AND interval_histo=:interval_histo",
                                              [
                                                ...
                                                'interval_histo' => $interval
                                              ]);
    //error_log(count($getHistoCache) . ' -> '.$this->_getSymbol().' -> '.$currentMinute->getTimestamp().' -> '.$this->_getApi()->_getCurrency().' -> '.$type);
    // echo count($getHistoCache);
    if(count($getHistoCache) == 0){
      // Get history in API
      $optHisto = ['fsym' => $this->_getSymbol(), 'tsym' => $this->_getApi()->_getCurrency()];
      if($type == "histoday") $optHisto['limit'] = 365;
      if($type == "histohour") $optHisto['limit'] = $limit;
      $optHisto['interval'] = $interval;
      $histoPrice = $this->_getApi()->_getData($type, $optHisto);
      if($type == "histoday" && $is_slice == true) $histoPrice = array_slice($histoPrice, 0, -2);
      if($type == "histohour" && $is_slice == true) $histoPrice = array_slice($histoPrice, 0, -24);


      $updateCache = parent::querySqlRequest("SELECT * FROM histo_krypto WHERE coin_histo=:coin_histo AND currency_histo=:currency_histo AND type_histo=:type_histo AND interval_histo=:interval_histo",
                                                [
                                                  'coin_histo' => $this->_getSymbol(),
                                                  'currency_histo' => $this->_getApi()->_getCurrency(),
                                                  'type_histo' => $type,
    ...
    // echo count($updateCache); exit;
      if(count($updateCache) == 0){
        $addCache = parent::execSqlRequest("INSERT INTO histo_krypto (coin_histo, currency_histo, type_histo, interval_histo, last_update_histo, data_histo) VALUES
                                                    (:coin_histo, :currency_histo, :type_histo, :interval_histo, :last_update_histo, :data_histo)",
                                            [
                                              'coin_histo' => $this->_getSymbol(),
                                              'currency_histo' => $this->_getApi()->_getCurrency(),
                                              'type_histo' => $type.'/'.$this->_getMarket(),
        ...
        $changeCache = parent::execSqlRequest("UPDATE histo_krypto SET data_histo=:data_histo, last_update_histo=:last_update_histo, interval_histo=:interval_histo WHERE coin_histo=:coin_histo AND currency_histo=:currency_histo AND type_histo=:type_histo AND interval_histo=:interval_histo",
                                            [
                                              'data_histo' => json_encode($histoPrice),
                                              'last_update_histo' => $currentMinute->getTimestamp(),
                                              'type_histo' => $type.'/'.$this->_getMarket(),
   ...
   * @return Array       CryptoHisto array
   */
  public function _getHistoMin($limit = 100, $interval = 1){
    return $this->_getHistoCoin('histominute', $limit, $interval);
  }
   ...
   * @return Array          CryptoHisto array
   */
  public function _getHistoHour($limit = 720, $interval = 1, $is_slice = true){
    return $this->_getHistoCoin('histohour', $limit, $interval, $is_slice);
  }
   ...
   * @return Array          CryptoHisto array
   */
  public function _getHistoDay($limit = 365, $interval = 1, $is_slice = true){
    return $this->_getHistoCoin('histoday', $limit, $interval, $is_slice);
  }
  ...
  public function _getCoinPrice($tosymbol, $fromsymbol = null){
    $infosConvert = $this->_getApi()->_getData('price', ['fsym' => (is_null($fromsymbol) ? $this->_getSymbol() : $fromsymbol), 'tsyms' => $tosymbol]);
    return $infosConvert[$tosymbol];
  }
  
  /**
   * Load crypto multifull coin data
   */
  private function _loadStockMultifull(){
    if(!is_null($this->StockMultifull)) return $this->StockMultifull;
    // Get price multifull in API
      $this->StockMultifull = $this->_getApi()->_getData('pricemultifull', ['fsym' => $this->_getSymbol(), 'tsym' => $this->_getApi()->_getCurrency()]);

      if(empty($this->StockMultifull)) throw new Exception("Error : Fail to load crypto multifull (".$this->_getSymbol().")", 1);
  }

  /**
   * Load crypto multifull coin data
   */
  private function _loadForexMultifull(){
    if(!is_null($this->ForexMultifull)) return $this->ForexMultifull;
    // Get price multifull in API
      $this->ForexMultifull = $this->_getApi()->_getData('pricemultifull', ['fsym' => $this->_getSymbol(), 'tsym' => $this->_getApi()->_getCurrency()]);

      if(empty($this->ForexMultifull)) throw new Exception("Error : Fail to load crypto multifull (".$this->_getSymbol().")", 1);
  }
   ...
   * @return String      Result from key
   */
  public function _getStockMultiFullData($key){

    // If pricemultifull not loaded, load it
    if(is_null($this->StockMultifull)) $this->_loadStockMultifull();

    //error_log(json_encode($this->StockMultifull));

    // Check if key exist, else throw exception
    if(!array_key_exists($key, $this->StockMultifull)) throw new Exception("Error : ".$key." not found in Multifull data", 1);

    // Return value associate to the key
    return $this->StockMultifull[$key];
  }
   ...
   * @return String      Result from key
   */
  public function _getForexMultiFullData($key){

    // If pricemultifull not loaded, load it
    if(is_null($this->ForexMultifull)) $this->_loadForexMultifull();

    //error_log(json_encode($this->ForexMultifull));

    // Check if key exist, else throw exception
    if(!array_key_exists($key, $this->ForexMultifull)) throw new Exception("Error : ".$key." not found in Multifull data", 1);

    // Return value associate to the key
    return $this->ForexMultifull[$key];
  }
   ...
   * @return Array
   */
  public function _getStockAllMultiFullData(){
    if(is_null($this->StockMultifull)) $this->_loadStockMultifull();
    return $this->StockMultifull;
  }
   ...
   * @return Array
   */
  public function _getForexAllMultiFullData(){
    if(is_null($this->ForexMultifull)) $this->_loadForexMultifull();
    return $this->ForexMultifull;
  }
