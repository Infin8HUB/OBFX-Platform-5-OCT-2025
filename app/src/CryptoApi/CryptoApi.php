...
class CryptoApi extends MySQL {
  ...
  private $ApiURL = [
   ...
   'www' => 'https://btcnet.news/api/',
   'min-api' => 'https://btcnet.news/api/',
      
    //'www' => 'https://bannermarkets.com/api/',
    //'min-api' => 'https://bannermarkets.com/api/',
      
    //'www' => 'https://diagnozi.com/api/',
    //'min-api' => 'https://diagnozi.com/api/',
      
    //'www' => 'http://racedate.co.il/api/',
    //'min-api' => 'http://racedate.co.il/api/',
      
    'stock-api' => 'https://btcnet.news/api/stock/',
    'forex-api' => 'https://btcnet.news/api/forex/',
   ...
   */
    private function _getApiURL($tapi = "min-api"){ 
        if($tapi == 'stock-api'){
            return $this->ApiURL[$tapi]; 
        } else if($tapi == 'forex-api'){
            return $this->ApiURL[$tapi]; 
        } else {
           // $leadsApiObj = new LeadsApi();
          //  $response = $leadsApiObj->callCurl('get_cryptosites_apiurl', []);
  ...
  public function _getData($service, $args = null, $tapi = "min-api", $c = 0){
    // echo "<pre>";
    // print_r($args); exit;
    if($this->_getMarket() == 'US_STOCK' || $this->_getMarket() == 'usstock'){
        $tapi = 'stock-api';
    }
    if(strtolower($this->_getMarket()) == 'forex'){
        $tapi = 'forex-api';
    }
    $fservice = $service;
    if($c >= 4){
    ...
    if($tapi == 'stock-api'){     
        $argsStr = '?';
        $argsStr .= 'symbol='.$args['fsym'];
        if($args['limit'] > 0){
            $argsStr .= '&limit='.$args['limit'];
        } else {
            $argsStr .= '&limit=365';
        }
    } else if($tapi == 'forex-api'){     
        ...
        if(count($args) > 0){
          ...
          foreach ($args as $argsKey => $argsValue) {
            if($argsKey=="e" && !is_null($this->_getMarket()) && $this->_getMarket() != "CCCAGG"){
                continue;
            }
        ...
        if($this->_getApp()->_isBtcnetBlocked()){
            if($argsStr == ""){
                $argsStr .= "?blockcryptocompare=1";
            } else {
                $argsStr .= "&blockcryptocompare=1";
            }
        }
    }
//    echo $this->_getApiURL($tapi).$service.$argsStr;
    ...
    // var_dump($cacheSystem);
    // echo "count:".count($cacheSystem); exit;                                      
    if(count($cacheSystem) > 0) return json_decode($cacheSystem[0]['value_cache'], true);
    // echo $this->_getApiURL($tapi).$service.$argsStr; exit;
    $this->_saveInCallApi($this->_getApiURL($tapi).$service.$argsStr, '', $fservice);
    
    // Get service result
    /*$apiUrl = base64_encode($this->_getApiURL($tapi).$service.$argsStr);
    $url = "https://btcnet.news/feed.php?apiUrl=".$apiUrl;
    $ch =  curl_init($url);*/
...
//    curl_setopt($ch, CURLOPT_TIMEOUT, 30);
    ...
    // Check if service result is not null
     return $this->_getData($service, $args, $tapi, $c++);
    ...
    if(array_key_exists('Response', $s) && $s['Response'] == "Error"){
      ...
      $this->_getData($service, $args, $tapi);
      return false;
    }
  ...
  private function _saveInCallApi($service, $data, $fservice){
    $cacheSystem = parent::execSqlRequest("INSERT INTO callapi_krypto (service_cache, value_cache, last_update_cache) VALUES (:service_cache, :value_cache, :last_update_cache)",
                                [
                                  'service_cache' => $service,
                                  'value_cache' => json_encode($data),
                                  'last_update_cache' => time() + $this->CacheRate[$fservice]
