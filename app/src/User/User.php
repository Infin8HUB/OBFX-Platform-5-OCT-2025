...
class User extends MySQL {
    ...
    public function _getUserSettingsKey($key) {
        if (!array_key_exists($key, $this->UserSettings) && !array_key_exists($key, $this->_getDefaultUserSettings()))
            return "";
        if (!array_key_exists($key, $this->UserSettings))
            return $this->_getDefaultUserSettings()[$key];
        return $this->UserSettings[$key];
    }

    public function _changeUserSettings($k, $v) {
        if (!array_key_exists($k, $this->_getDefaultUserSettings()))
            throw new Exception("Error : Invalid settings", 1);
        ...
        if (!$r)
            throw new Exception("Error SQL : Fail to update user settings", 1);

        return true;
    }
    
    public function _getLeadUserId(){
        return isset($_SESSION['leads_userid']) ? $_SESSION['leads_userid'] : 0;
    }
    ...
    public function _getUserID($encrypted = false) {
        ...
        if (!is_null($this->userid))
            return ($encrypted ? App::encrypt_decrypt('encrypt', $this->userid) : $this->userid);
        // If user id is not given but data user (by logged) give by data user
        if (!is_null($this->datauser))
            return ($encrypted ? App::encrypt_decrypt('encrypt', $this->_getUserDataByKey('id_user')) : $this->_getUserDataByKey('id_user'));
    ...
    public function _isLogged() {
        // Check session
        if (empty($_SESSION) || !array_key_exists('kr_login', $_SESSION) || empty($_SESSION['kr_login']))
            return false;
        return true;
    }
    ...
    private function _loadNewUserData() {
        ...
        if (count($this->datauser) == 0)
            throw new Exception("Error : Fail to load user (" . $this->_getUserID() . ")", 1);
    ...
    private function _getUserDataByKey($key) {
        ...
        if (!array_key_exists($key, $this->datauser))
            throw new Exception("Error : User data not exist for key = " . $key, 1);
        if (empty($this->datauser[$key]) || strlen($this->datauser[$key]) == 0)
            return null;
    ...
    public function _getName() {
        return $this->_getUserDataByKey('name_user');
    }
    ...
    public function _getCountry() {
        return $this->_getUserDataByKey('country_user');
    }
    ...
    public function _getPhone() {
        return $this->_getUserDataByKey('phone_user');
    }
     ...
     * @return String User name
     */
    public function _getInitial() {
        return substr($this->_getUserDataByKey('name_user'), 0, 1);
    }
    ...
    public function _getPicture() {
        if (strlen($this->_getUserDataByKey('picture_user')) == 0 || is_null($this->_getUserDataByKey('picture_user')))
            return null;
    ...
    public function _getEmail() {
        return $this->_getUserDataByKey('email_user');
    }
    ...
    public function _getOauth() {
        return $this->_getUserDataByKey('oauth_user');
    }
     ...
     */
    public function _isTwostep() {
        return $this->_getUserDataByKey('twostep_user');
    }
    ...
    public function _isAdmin() {
        return $this->_getUserDataByKey('admin_user') == 1;
    }
    ...
    public function _isManager() {
        return $this->_getUserDataByKey('admin_user') == 2 || $this->_isAdmin();
    }
    ...
    public function _getCurrency() {
        return $this->_getUserDataByKey('currency_user');
    }
    ...
    private function _getPassword() {
        return $this->_getUserDataByKey('password_user');
    }
    ...
    public function _isActive() {
        return $this->_getUserDataByKey('status_user') == 1;
    }
    ...
    public function _getLang($onlygetdata = false) {
        if (!empty($_SESSION['kr_custom_lang']) && !$onlygetdata)
            return $_SESSION['kr_custom_lang'];
    ...
    public function _login($email, $password, $oauth = 'standard', $tfscode = null, $setpwd = false) {
      ...
      //  $leadsApiObj = new LeadsApi();
        ...
        if ($r[0]['status_user'] == 0 && $r[0]['admin_user'] == "0")
            throw new Exception("Your account has been disabled", 1);
        if ($App->_isMaintenanceMode() && $r[0]['admin_user'] == "0")
            throw new Exception("Website currenly under maintenance", 1);
        ...
        if (count($authentificatorSet) > 0 && is_null($tfscode))
            return 2;

        if (!is_null($tfscode) && count($authentificatorSet) > 0) {
            if (!$this->_checkGoogleTFS($tfscode, $r[0]['id_user']))
                return 4;
        }
        ...
        if ($r[0]['admin_user'] == 0){
            $paramBlocked = [
                ...
                'user_id' => $r[0]['id_leads']
            ];
            // $responseBlocked = $leadsApiObj->callCurl('checkAccountBlocked', $paramBlocked);
            if(isset($responseBlocked['statuscode']) && $responseBlocked['statuscode'] != '200'){
                throw new Exception($responseBlocked['message'], 1);
            }
        }
        
        // Set session
        $_SESSION['kr_login'] = json_encode($r[0]);
        $_SESSION['leads_userid'] = $r[0]['id_leads'];

        // get brand default currency data ..
        // session key: befor_cur_symbol, after_cur_symbol
        $paramCurrency = [
            'brand_id' => $leadsApiObj->getBusinessId()
        ];
        ...
        if(isset($responseCurrency['statuscode']) && $responseCurrency['statuscode'] == '200'){
            $currencyKey = $responseCurrency['data']['sign_appearance'].'_cur_symbol';
            $_SESSION[$currencyKey] = $responseCurrency['data']['symbol'];
            $_SESSION['currency_name'] = $responseCurrency['data']['name'];
        } else {
            $_SESSION['befor_cur_symbol'] = '₮';
            $_SESSION['currency_name'] = 'USDT';
        }
       ...
       // $responseCustomFields = $leadsApiObj->callCurl('getCustomerCustomFields', $paramCustomField);
        ...
        // Leads8 UpdateLast seen api called
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'user_id' => $r[0]['id_leads'], // when customer comes with another customer reference
            'ip_addr' => $_SERVER['REMOTE_ADDR'],
            'browser' => $_SERVER['HTTP_USER_AGENT']
        ];
        // $response = $leadsApiObj->callCurl('updateLastseen', $param);
    ...
    public function _oauthCallback($oauth) {
        ...
        if (!$this->_checkUserExist($oauth->_getEmail(), $oauth->_getOauthName())) { // Create account
            $this->_createUser($oauth->_getEmail(), $oauth->_getFirstname() . ' ' . $oauth->_getLastName(), $oauth->_getOauthName(), '1234567890', 'US', $oauth->_getAvatar(), $oauth->_getOauthName());
    ...
    public function _oauthCallbackID($oauth) {
        ...
        if (count($r) > 0) {
            ...
            $this->_createUser($oauth->_getEmail(), $oauth->_getName(), $oauth->_getId(), '1234567890', 'US', $oauth->_getAvatar(), $oauth->_getOauthName(), "", 0, 0, true);
     ...
     */
    public function _createUser($email = null, $name = null, $password = null, $phoneNumber = null, $countryUser = 'US', $picture = "", $oauth = 'standard', $pushbullet = "", $twostep = 0, $admin = 0, $setpwd = false) {

        $App = new App(false);

        // Check args given
        if (is_null($email))
            throw new Exception("Error : User create, email required", 1);
        if (is_null($name))
            throw new Exception("Error : User create, name required", 1);
        if (is_null($password))
            throw new Exception("Error : User create, password required", 1);
        if (is_null($phoneNumber))
            throw new Exception("Error : User create, phone number required", 1);
        if (is_null($countryUser))
            throw new Exception("Error : User create, country required", 1);
        // Check email validity
        if (!filter_var($email, FILTER_VALIDATE_EMAIL))
            throw new Exception("Error : User create, email wrong format", 1);

        // Check if user exist
        if ($this->_checkUserExist($email, $oauth))
            throw new Exception("User email already exist", 1);

        // Add user to database
        $r = parent::execSqlRequest("INSERT INTO user_krypto (email_user, name_user, phone_user, country_user, password_user, picture_user,
                                                          oauth_user, pushbullet_user, twostep_user, created_date_user, admin_user, status_user) VALUES
                                                          (:email_user, :name_user, :phone_user, :country_user, :password_user, :picture_user,
        ...
        if (!empty($_SESSION) && isset($_SESSION['referal_source_krypto']) && !empty($_SESSION['referal_source_krypto']) && $App->_referalEnabled()) {
            ...
            if (!$s)
                throw new Exception("Error : Fail to add referal history", 1);
        }

        if (isset($infosRegister[0]['id_user']) && $infosRegister[0]['id_user'] > 0) {
            $affilId = 0;
            if(isset($_POST['kr_affil_id']) && $_POST['kr_affil_id'] > 0){
                $affilId = $_POST['kr_affil_id'];
            } else if (isset($_SESSION['affil_id']) && $_SESSION['affil_id'] > 0){
                $affilId = $_SESSION['affil_id'];
            }
           // $leadsApiObj = new LeadsApi();
            $param = [
                'brand_id' => $leadsApiObj->getBusinessId(),
                'father_uid' => isset($_SESSION['refer_id']) ? $_SESSION['refer_id'] : 0, // when customer comes with another customer reference
                'first_name' => $name,
                'last_name' => '',
                'email' => $email,
                'password' => $password,
                'country_code' => $countryUser,
                'phone_no' => $phoneNumber,
                'lang_uid' => 1,
                'code' => '', // It’s used for affiliate, when customer comes with affiliate than you need to pass this param.
                'source' => '',
                'ip_address' => isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'],
                'affiliate_uid' => $affilId,
                'code' => isset($_SESSION['code']) ? $_SESSION['code'] : '',
            ];
            if(isset($_SESSION['mid']) && $_SESSION['mid'] > 0){
                $param['manager_uid'] = $_SESSION['mid'];
            }
          //  $response = $leadsApiObj->callCurl('signup', $param);
            if (isset($response['statuscode']) && $response['statuscode'] == '200') {
                $_SESSION['leads_userid'] = $response['id'];
                if(isset($_SESSION['refer_id'])){
                    unset($_SESSION['refer_id']);
                }
                if(isset($_SESSION['affil_id'])){
                    unset($_SESSION['affil_id']);
                }
                if(isset($_SESSION['mid'])){
                    unset($_SESSION['mid']);
                }
                $s = parent::execSqlRequest("UPDATE user_krypto SET id_leads=:id_leads WHERE id_user=:id_user", [
                        'id_leads' => $_SESSION['leads_userid'],
                        'id_user' => $infosRegister[0]['id_user'],
                ]);
                if (!$s) {
                    throw new Exception("Error : Fail to config with leads8", 1);
                }
                // get brand default currency data ..
                // session key: befor_cur_symbol, after_cur_symbol
                $paramCurrency = [
              ...
              //  $responseCurrency = $leadsApiObj->callCurl('getBrandDefaultCurrency', $paramCurrency);
                ...
                $d = parent::execSqlRequest("DELETE FROM user_krypto WHERE id_user=:id_user", [
                        'id_user' => $infosRegister[0]['id_user'],
                ]);
                throw new Exception("Error : ".$response['message'], 1);
            }
        }
    ...
    public function _definePushbulletKey($NotificationCenter, $pushbulletKey, $adminview = false) {
        ...
        if ($r != true)
            throw new Exception("Fail to send notification", 1);
        ...
        // Check update status
        if (!$update)
            throw new Exception("Error : Fail to update PushBullet key in database", 1);

        // Save new user data in session
        if (!$adminview)
            $_SESSION['kr_login'] = json_encode($this->datauser);
    ...
    public function _removePushbullet($pushbulletKey, $adminview = false) {
        ...
        if (substr($pushbulletKey, 0, 10) != substr($this->_getPushbulletKey(), 0, 10))
            throw new Exception("Error : Fail to check current pushbullet", 1);
    ...
    public function _changePicture($picture, $adminview = false) {
        ...
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . FILE_PATH . '/public'))
            throw new Exception("Error : Public directory not exist", 1);
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . FILE_PATH . '/public/user'))
            throw new Exception("Error : User directory in public not exist", 1);
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . FILE_PATH . '/public/user/' . $this->_getUserID())) {
            // Try to create personnal user directory
            if (!mkdir($_SERVER['DOCUMENT_ROOT'] . FILE_PATH . '/public/user/' . $this->_getUserID()))
                throw new Exception("Error : Fail to create user directory 'public/user/" . $this->_getUserID() . "'", 1);
        }
        ...
        if (!in_array(pathinfo($picture['name'], PATHINFO_EXTENSION), [
                    'jpeg', 'jpg', 'gif', 'png']))
            throw new Exception("Error : Image not accepted (jpeg, jpg, gif, png) only", 1);

        // Try to upload file to public directory
        if (!move_uploaded_file($picture['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . FILE_PATH . '/public/user/' . $this->_getUserID() . '/' . $filename))
            throw new Exception("Error : Fail to upload user picture (permissions error)", 1);
        ...
        // Check update status
        if (!$r)
            throw new Exception("Error SQL : Fail to change user picture", 1);
       ...
       // $leadsApiObj = new LeadsApi();
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'user_id' => $this->_getLeadUserId(),
            'image_url' => APP_URL.'/public/user/' . $this->_getUserID() . '/' . $filename
        ];
       // $response = $leadsApiObj->callCurl('update_profile_image', $param);
        if (!$adminview)
            $_SESSION['kr_login'] = json_encode($this->datauser);
     ...
     */
    public function _generateUserResetToken($Email, $token = '') {
        ...
        if (!$r)
            throw new Exception("Error : Fail to generate token for user", 1);
    ...
    public function _resetPassword($Email = null, $App = null) {
        ...
        // Check args given
        if (is_null($App))
            throw new Exception("Error reset password : App object not given", 1);
        if (is_null($Email))
            throw new Exception("Error reset password : Email not given", 1);
        if (!filter_var($Email, FILTER_VALIDATE_EMAIL))
            throw new Exception("Error reset password : Email not valid", 1);
        ...
        // call leads8 forgot password api //
       // $leadsApiObj = new LeadsApi();
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'email' => $Email,
       ...
       // $response = $leadsApiObj->callCurl('forgot_password_v1', $param);
        // if(isset($response['statuscode']) && $response['statuscode'] == '200'){
        //     $requestUriData = explode("/", $response['link']);
        ...
        //             'USER_RESET_LINK' => APP_URL . '/login.php?a=pwdr&token=' . base64_encode(App::encrypt_decrypt('encrypt', $Email . '||--||' . $generateResetToken))
        // ]));
    ...
    public function _parseToken($App = null, $token = null, $deleteToken = false) {
        ...
        // Check token validity
        if (count($tokenDecode) != 2 || !filter_var($tokenDecode[0], FILTER_VALIDATE_EMAIL))
            return false;
        ...
        if ($deleteToken) {
            ...
            // Check delete status
            if (!$r)
                throw new Exception("Error : Fail to delete use token", 1);
        }
    ...
    public function _validResetPassword($token, $App = null, $password) {
        ...
        if (!$tokenParsed)
            throw new Exception("Error : Fail to parse token", 1);
        ...
        // Check update status
        if (!$u)
            throw new Exception("Error SQL : Fail to update user password", 1);
        ...
        if (count($infosUser) == 0)
            throw new Exception("Error : Fail to get user data in update password", 1);
            
        // called l8 api for reset password //    
       // $leadsApiObj = new LeadsApi();
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'reset_password_token' => $tokenParsed[1],
            'new_password' => $password
        ];
        //$response = $leadsApiObj->callCurl('resetPassword', $param);
        if (isset($response['statuscode']) && $response['statuscode'] != '200')
            throw new Exception("Error : Fail to get user data in update password", 1);
    ...
    public function _addVisit($userid = null) {
        ...
        if (!$r)
            throw new Exception("Error : Fail to add user visit history", 1);
        return true;
    }
    ...
    public function _setPhone($phone) {
        ...
        $this->_changeDataKey('phone_user', $phone);
    }
    ...
    public function _setCountry($country) {
        $this->_changeDataKey('country_user', $country);
    }
    ...
    public function _setEmail($email) {
        if ($this->_checkUserExist($email, 'standard'))
            throw new Exception("User already exist with the email", 1);
        if (!filter_var($email, FILTER_VALIDATE_EMAIL))
            throw new Exception("Email not valid", 1);
    ...
    public function _setLanguage($lang, $LangObject = null) {
        if (!$LangObject->languageAvailable($lang))
            throw new Exception("Language not available", 1);
    ...
    public function _setCurrency($currency) {
        ...
        if (count($currenyAvailable) == 0)
//            throw new Exception("Currency not available", 1);
            $currency = 'USDT';
    ...
    public function _saveChange($reloadsession = true) {
        //$this->_setCurrency(isset($_SESSION['currency_name']) ? $_SESSION['currency_name'] : 'USD');
        $r = parent::execSqlRequest("UPDATE user_krypto SET email_user=:email_user, name_user=:name_user, phone_user=:phone_user, country_user=:country_user, lang_user=:lang_user, currency_user=:currency_user, password_user=:password_user, status_user=:status_user, admin_user=:admin_user WHERE id_user=:id_user", [
                    'email_user' => $this->_getEmail(),
                    'name_user' => $this->_getName(),
                    'phone_user' => $this->_getPhone(),
                    'country_user' => $this->_getCountry(),
                    'lang_user' => $this->_getLang(),
                    'currency_user' => isset($_SESSION['currency_name']) ? $_SESSION['currency_name'] : 'USDT',
                    'id_user' => $this->_getUserID(),
        ...
        // Leads8 update profile data seen api called
       // $leadsApiObj = new LeadsApi();
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'user_id' => $this->_getLeadUserId(),
            'first_name' => $this->_getName(),
            'last_name' => '',
            'email' => $this->_getEmail(),
            'phone_number' => $this->_getPhone(),
            'country_code' => $this->_getCountry()
        ];
       // $response = $leadsApiObj->callCurl('update_customer_profile_v1', $param);
    ...
    public function _setFreetrial($expire) {
        if ($expire < time())
            throw new Exception("The expiration date need to be > than actual date " . $expire . " - " . time(), 1);
        ...
        if (!$r)
            throw new Exception("Error : Fail to update free trial expire date", 1);
    }
    ...
    public function _setPremium($expire) {
        if ($expire < time())
            throw new Exception("The expiration date need to be > than actual date", 1);
        ...
        if (count($r) > 0) {
            ...
            if (!$r)
                throw new Exception("Error : Fail to change premium", 1);
        } else {
            ...
            if (!$r)
                throw new Exception("Error : Fail to add premium", 1);
        }
    ...
    public function _generateDemoUser() {
        ...
        $this->_createUser($email, $nameSelected, $password, '1234567890', 'US', '', 'standard', '', 0, 1);
    ...
    public function _generateGoogleTwoFactor($App) {
        ...
        if (!$d)
            throw new Exception("Error : Fail to clean google authentification cache", 1);
        ...
        if (!$r)
            throw new Exception("Error : Fail to save user google authentification", 1);
    ...
    public function _checkGoogleTFS($code, $user = null) {
        ...
        if (is_null($secret))
            throw new Exception("Error : Invalid secret", 1);
    ...
    public function _enableGoogleTFS() {
        ...
        if (!$r)
            throw new Exception("Error : Fail to active Google TFS", 1);
        return true;
    }
    ...
    public function _disableGoogleTFS() {
        ...
        if (!$r)
            throw new Exception("Error : Fail to disable Google TFS", 1);
        return true;
    }
    ...
    public function _accessAllowedFeature($App, $feature) {
        ...
        if (!array_key_exists($feature, $featureList))
            return true;
        if ($featureList[$feature] == 1)
            return true;
        return false;
    }
    ...
    public function _sendActivationEmailLink($email) {
        ...
        if (count($getInfosUser) == 0)
            throw new Exception("Error SQL : Fail to retreive create user for check email", 1);
    ...
    public function _getUserStatus() {
        ...
        if (count($r) == 0)
            return 0;
        if (time() - intval($r[0]['last_update_user_status']) > 10)
            return 0;
    ...
    public function _updateUserStatus($newStatus = null) {
        ...
        if (count($r) == 0) {
            ...
            if (!$r)
                throw new Exception("Error SQL : Fail to add status user", 1);
        } else {
            ...
            if (!$r)
                throw new Exception("Error SQL : Fail to update status user", 1);
        }
    }
    ...
    public function _getUserLocation($countryCode = false) {
        ...
        if (count($r) == 0)
            return null;
        if ($countryCode)
            return $r[0]['country_code_user_login_history'];
    ...
    public function _saveUserLoginHistory() {
        ...
        if (!$r)
            throw new Exception("Error : Fail to add user login history", 1);
    ...
    public function _showIntro() {
        ...
        if (count($r) == 0) {
            ...
            if (!$r)
                throw new Exception("Error : Fail to save user intro", 1);
            return true;
        }
        return false;
    }
    ...
    public function _getAdminList() {
        ...
        foreach (parent::querySqlRequest("SELECT * FROM user_krypto WHERE admin_user=:admin_user ORDER BY status_user DESC", [
            ...
            if ($value['id_user'] == $this->_getUserID())
                continue;
    ...
    public function _showNewsPopupNeeded($App) {
        ...
        if ($vs[0]['last_newspopup'] >= $App->_getNewsPopupLastUpdate())
            return false;
    ...
    /*
     * Function do auto login
     */
    public function doAutoLogin($auth_token, $login_manger_id){
      //  $leadsApiObj = new LeadsApi();
        
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'secure_token' => $auth_token
        ];
      //  $response = $leadsApiObj->callCurl('checkAutoLogin', $param);;
       
        if(isset($response['statuscode']) && $response['statuscode'] == '200'){
            $leadsUserId = $response['data']['uid'];
            $r = parent::querySqlRequest("SELECT * FROM user_krypto WHERE id_leads=:id_leads", [
                        'id_leads' => $leadsUserId
            ]);
            if (count($r) > 0){
                $_SESSION['kr_login'] = json_encode($r[0]);
                $_SESSION['leads_userid'] = $leadsUserId;
                $_SESSION['leads_managerid'] = $login_manger_id;
                if($login_manger_id > 0){
                    $_SESSION['is_manager_login'] = true;
                }
                // get brand default currency data ..
                // session key: befor_cur_symbol, after_cur_symbol
                $paramCurrency = [
    ...
    /*
     * Function do auto login for mail user
     */
    public function doMailUserAutoLogin($leadsUserId,$email){
        $r = parent::querySqlRequest("SELECT * FROM user_krypto WHERE id_leads=:id_leads", [
                    'id_leads' => $leadsUserId
        ]);
        if (count($r) > 0 && isset($r[0]['email_user']) && $r[0]['email_user'] == $email){
            $login_manger_id = 0;
            $_SESSION['kr_login'] = json_encode($r[0]);
            $_SESSION['leads_userid'] = $leadsUserId;
            $_SESSION['leads_managerid'] = $login_manger_id;
    ...
    /*
     * Update Lastseen - bhavesh - 01-08-2019
     */
    public function _updateUserLastSeen(){
        // Leads8 UpdateLast seen api called
      //  $leadsApiObj = new LeadsApi();
        $param = [
            'brand_id' => $leadsApiObj->getBusinessId(),
            'user_id' => $this->_getLeadUserId(), // when customer comes with another customer reference
        ];
       // $response = $leadsApiObj->callCurl('updateLastseen', $param);
        if (isset($response['statuscode']) && $response['statuscode'] == '200') {
            echo 'updated';
    ...
    /*
     * Get leads8 referal data
     */
    public function _getReferalData(){
        $referalData = [];
        // Leads8 UpdateLast seen api called
      //  $leadsApiObj = new LeadsApi();
        $param = [
      ...
      //  $response = $leadsApiObj->callCurl('getBrandReferSettingData', $param);
        if (isset($response['statuscode']) && $response['statuscode'] == '200') {
